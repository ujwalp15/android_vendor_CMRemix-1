function __print_cmremix_functions_help() {
cat <<EOF
...
function breakfast()
{
    target=$1
    local variant=$2
    CMREMIX_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/cmremix/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the CMREMIX model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var CMREMIX_VERSION)
        ZIPFILE=cmremix-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
...
function dopush()
{
    ...
    if (adb shell getprop ro.cmremix.device | grep -q "$CMREMIX_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/cmremix/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $CMREMIX_FIXUP_COMMON_OUT ]; then
